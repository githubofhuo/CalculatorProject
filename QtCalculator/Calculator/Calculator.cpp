...
Calculator::Calculator( QWidget * parent): QMainWindow(parent)
{
    ...
    setupUi(this);
    this->setWindowTitle(tr("Science Calculator"));
    readSettings();
    //默认的进制为十进制
...
void Calculator::writeSettings()
{
    ...
    if(isChangeColor)
    {
        ...
        ret = QMessageBox::question(this,tr("科学计算器"),tr("Background color,do you want to save it?"),
    ...
    {
        QMessageBox::StandardButton ret;
        ret = QMessageBox::question(this,tr("科学计算器"),tr("Font has been changes,do you want to save it?"),
                              QMessageBox::Save| QMessageBox::Cancel);
        if (ret != QMessageBox::Save){
             QFont font("Arial",20);
